########## This code applies EmtpyDrops default settings to find empty droplets ##########
...
# cell_determination = EmptyDrops 

##### run emptyDrops by cartridge/experiment and extract counts from real cells from NAT and tumor samples --> 0.01 FDR cutoff 
### P1 
...
## assign barcodes based on Sample Tag Calls (all barcodes before putative cell calling)
...
###### plot example plot from empty drops result highlighting eosinophils
## rerun 
df <- emptyDrops(counts_data,test.ambient = TRUE)
df$is.cell <- df$FDR <= 0.01
table(df$is.cell) 
...
# extract eosinophil specific barcodes 
...
# Optionally, make special points slightly bigger
