...
#obj <- subset(obj, idents = c("B","DCs","Endothelial","Eosinophils","Epithelial","Fibroblasts","Macrophages","Mast","Monocytes","Neutrophils","PCs","T","TAMs"))
...
nExp_poi <- round(0.05*nrow(sub@meta.data))  
nExp_poi.adj <- round(nExp_poi*(1-homotypic.prop))

## Run DoubletFinder with varying classification stringencies ----------------------------------------------------------------
sub <- doubletFinder(sub, PCs = 1:10, pN = 0.25, pK = 0.25, nExp = nExp_poi, reuse.pANN = NULL, sct = FALSE)
sub$DoubletFinder_score <- sub$pANN_0.25_0.25_798
sub$DoubletFinder_class <- sub$DF.classifications_0.25_0.25_798
sub_exp1 <- sub
...
## Run DoubletFinder with varying classification stringencies ----------------------------------------------------------------
sub <- doubletFinder(sub, PCs = 1:10, pN = 0.25, pK = 0.28, nExp = nExp_poi, reuse.pANN = NULL, sct = FALSE)
sub$DoubletFinder_score <- sub$pANN_0.25_0.28_854
sub$DoubletFinder_class <- sub$DF.classifications_0.25_0.28_854
sub_exp2 <- sub
...
## Run DoubletFinder with varying classification stringencies ----------------------------------------------------------------
sub <- doubletFinder(sub, PCs = 1:10, pN = 0.25, pK = 0.07, nExp = nExp_poi, reuse.pANN = NULL, sct = FALSE)
sub$DoubletFinder_score <- sub$pANN_0.25_0.07_507
sub$DoubletFinder_class <- sub$DF.classifications_0.25_0.07_507
sub_exp3 <- sub
...
## Run DoubletFinder with varying classification stringencies ----------------------------------------------------------------
sub <- doubletFinder(sub, PCs = 1:10, pN = 0.25, pK = 0.005, nExp = nExp_poi, reuse.pANN = NULL, sct = FALSE)
sub$DoubletFinder_score <- sub$pANN_0.25_0.005_454
sub$DoubletFinder_class <- sub$DF.classifications_0.25_0.005_454
sub_exp4<- sub
...
## Run DoubletFinder with varying classification stringencies ----------------------------------------------------------------
sub <- doubletFinder(sub, PCs = 1:10, pN = 0.25, pK = 0.29, nExp = nExp_poi, reuse.pANN = NULL, sct = FALSE)
sub$DoubletFinder_score <- sub$pANN_0.25_0.29_1197
sub$DoubletFinder_class <- sub$DF.classifications_0.25_0.29_1197
sub_exp5 <- sub
...
## Run DoubletFinder with varying classification stringencies ----------------------------------------------------------------
sub <- doubletFinder(sub, PCs = 1:10, pN = 0.25, pK = 0.23, nExp = nExp_poi, reuse.pANN = NULL, sct = FALSE)
sub$DoubletFinder_score <- sub$pANN_0.25_0.23_1109
sub$DoubletFinder_class <- sub$DF.classifications_0.25_0.23_1109
sub_exp7 <- sub
...
## Run DoubletFinder with varying classification stringencies ----------------------------------------------------------------
sub <- doubletFinder(sub, PCs = 1:10, pN = 0.25, pK = 0.2, nExp = nExp_poi, reuse.pANN = NULL, sct = FALSE)
sub$DoubletFinder_score <- sub$pANN_0.25_0.2_1315
sub$DoubletFinder_class <- sub$DF.classifications_0.25_0.2_1315
sub_exp8 <- sub
...
ggsave("/scratch/khandl/technical/figures/Doublet/DoubletFinder_wo_GT_umap.svg", width = 8, height = 8, plot = p)
...
multiplet_rate_per_sample <- c(4.999374,4.997659,4.999507,4.996148,4.998956,5.001804,5.001331)
df <- data.frame(sample, multiplet_rate_per_sample)
df$method <- "DoubletFinder"
write.csv(df,"/scratch/khandl/technical/figures/Doublet/DoubletFinder_doublet_rate.csv")
...
write.csv(deconvolution_crc_df,"/scratch/khandl/technical/figures/Doublet/DoubletFinder_wo_doublets_deconvolution_result.csv")
