...
DoubletFinder_df <- read.csv("/scratch/khandl/technical/figures/Doublet/DoubletFinder_wo_doublets_deconvolution_result.csv")
...
for (i in sample){
  ...
  df <- sub@meta.data %>%
    group_by(method = .data$method) %>%
    summarise(mean_SCDC_eos = mean(Neutrophils, na.rm = TRUE)) %>%
    as.data.frame()
  df$sample <- i
  df_list[[i]] <- df
}
...
for (i in sample){
  ...
  df <- sub@meta.data %>%
    group_by(method = .data$method) %>%
    summarise(mean_SCDC_eos = mean(Macrophages, na.rm = TRUE)) %>%
    as.data.frame()
  df$sample <- i
  df_list[[i]] <- df
}
...
for (i in sample){
  ...
  df <- sub@meta.data %>%
    group_by(method = .data$method) %>%
    summarise(mean_SCDC_eos = mean(T, na.rm = TRUE)) %>%
    as.data.frame()
  df$sample <- i
  df_list[[i]] <- df
}
...
p <- ggplot(df, aes(x = method, y =  mean_SCDC_eos, fill = method)) + 
  ...
  geom_point(position = position_jitterdodge(jitter.width = 0.3, dodge.width = 0.75), size = 2.5, shape = 21)+ 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
  scale_fill_manual(values =  c( "Cell_hashing" = "#09BEDB", "scDblFinder" = "#09BEDB","gene_counts" = "#09BEDB","DoubletFinder"="#09BEDB"))
ggsave("/scratch/khandl/technical/figures/Doublet/SCDC_score_T_across_methods.svg", width = 10, height = 8, plot = p)
