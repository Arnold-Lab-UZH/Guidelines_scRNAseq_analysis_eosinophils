...
### creation of Seurat object with human data 
...
create_seurat_Mm_datafrom_sparse_matrix <- function(
    input_matrix,
    project,
    condition, 
    min.cells,
...
### Functions to pre-process publicly available data
create_seurat_public_data <- function(
    path_to_data_file,
    project,
    min.cells,
    min.features,
...
create_seurat_zUMI_outputs_public <- function(
    count_matrix,
    project,
    min.cells,
    min.features,
...
### function to generate cell-gene count matrix as txt file for input to SCALPEL
SCALPEL_input_matrix_generation <- function(path_to_st_file,output_file_name_whitelist_bc, output_file_name_count_matrix) {
  df <- data_to_sparse_matrix(path_to_st_file)
  df <- as.data.frame(df)
  whitelist_bc <- colnames(df)
  whitelist_bc <- as.data.frame(whitelist_bc)
  colnames(whitelist_bc) <- NULL
  df$GENE <- rownames(df)
  df <- df[, c("GENE", setdiff(names(df), "GENE"))]
  rownames(df) <- NULL
  write.table(whitelist_bc, file = output_file_name_whitelist_bc, sep = "\t", quote = FALSE, row.names = FALSE)
  write.table(df, file = output_file_name_count_matrix, sep = "\t", quote = FALSE, row.names = FALSE)
}
